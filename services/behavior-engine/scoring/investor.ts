...
// services/behavior-engine/scoring/investor.ts

export function scoreInvestor(event: any) {
  // Basic placeholder scoring logic — replace with your real model later
  const base = 50;

  const impulsiveness = event.impulsivenessScore ?? 0;
  const pageWeight = event.page === "investment" ? 20 : 0;

  return base + impulsiveness + pageWeight;
}
