...
export default function BehaviorDetailPage({ params }: { params: { id: string } }) {
  const { id } = params;
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        console.error("Behavior page error:", err);
      } finally {
        setLoading(false);
      }
    }
  ...
  if (!profile) {
    return <div className="p-10 text-xl">No behavior profile found.</div>;
  }

  return (
    <div className="min-h-screen p-10 bg-gray-100 dark:bg-[#0f0f0f] text-black dark:text-white space-y-10">
      <h1 className="text-3xl font-bold">
        Behavior Detail — {profile.user?.email || profile.lead?.email || profile.investor?.email || "Unknown"}
      </h1>
